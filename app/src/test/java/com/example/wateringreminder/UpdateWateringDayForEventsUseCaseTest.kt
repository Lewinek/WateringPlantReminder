...
class UpdateWateringDayForEventsUseCaseTest {
    ...
    @Test
    fun `GIVEN events WHEN invoke THEN updateEvents should be called with corrected arguments`() = runTest {
        // Arrange
        val events = listOf(
            Event(
                id = 1,
    ...
    @Test
    fun `GIVEN events WHEN invoke THEN updateWateringInfo should return updated arguments`() = runTest{
        val events = listOf(
            Event(
                id = 1,
                wateringDate = LocalDate.parse("2023-03-29"),
